...
function shift_subject_position($start_pos, $end_pos, $current_id=0) {
  ...
  if($start_pos == 0) {
    ...
    $sql .= "WHERE position >= '" . db_scape($db, $end_pos) . "' ";
  } elseif($end_pos == 0) {
    //delete item, -1 from items greater that $start_pos
    $sql .= "SET position = position - 1 ";
    $sql .= "WHERE position > '" . db_scape($db, $start_pos) . "' ";
  } elseif($start_pos < $end_pos) {
    //move later, -1 from items between (including $end_pos)
    $sql .= "SET position = position - 1 ";
    $sql .= "WHERE position > '" . db_scape($db, $start_pos) . "' ";
    $sql .= "AND position <= '" . db_scape($db, $end_pos) . "' ";
  } elseif($start_pos > $end_pos) {
    $sql .= "SET position = position + 1 ";
    $sql .= "WHERE position >= '" . db_scape($db, $end_pos) . "' ";
    $sql .= "AND position < '" . db_scape($db, $star_pos) . "' ";
  }
...
function update_page($page) {
  ...
  shift_page_position($old_position, $page['position'], $page['subject_id'], $page['id']);
...
function delete_page($id) {
  ...
  $old_page = find_page_by_id($page['id']);
  $old_position = $old_page['position'];
  shift_page_position($old_position, 0, $old_page['subject_id'], $id);
...
function shift_page_position($start_pos, $end_pos, $subject_id, $current_id=0) {
  ...
  //Exclude the current_id in the SQL WHERE clause
  $sql .= "AND id != '" . db_scape($db, $current_id) . "' ";
  $sql .= "AND subject_id != '" . db_scape($db, $subject_id) . "'";
...
function find_admin_by_username($username) {
  ...
  $sql = "SELECT * FROM admins ";
  $sql .= "WHERE username='" . db_scape($db, $username) . "'";
  $result = mysqli_query($db, $sql);
  confirm_result_set($result);
  $admin = mysqli_fetch_assoc($result);
  mysqli_free_result($result);
...
function insert_admin($admin) {
  ...
  $sql .= "VALUES (";
  $sql .= "'" . db_scape($db, $admin['first_name']) . "', ";
  $sql .= "'" . db_scape($db, $admin['last_name']) . "', ";
  $sql .= "'" . db_scape($db, $admin['email']) . "', ";
  $sql .= "'" . db_scape($db, $admin['username']) . "', ";
  $sql .= "'" . db_scape($db, $hashed_password) . "'";
  $sql .= ")";
...
function update_admin($admin) { 
  ...
  if($password_sent) {
    $sql .= "hashed_password='" . db_scape($db, $hashed_password) . "', ";
  }
